...
const SkillLists: React.FC<Props> = ({
  ...
  return !descriptionDescription ? (
    <ul className="flex flex-col [counter-reset:section]">
      {descriptionTitle.map((title, index) => (
        <li
          key={index}
          className="mb-2 [counter-increment:section] text-base text-black font-bold before:content-[counters(section,'.')] before:text-base before:mr-2 before:inline-block"
        >
          {title}
        </li>
      ))}
